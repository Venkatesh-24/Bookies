...
function displayTable(page) {
  ...
  for (let i = startIndex; i < endIndex && i < currentBooks.length; i++) {
    ...
    row.innerHTML =`
      <div>
      <td style= "padding-top: 20px; padding-right:50px; padding-bottom: 80px; text-align: center;"><img src="${thumbnail}" alt="${title}" />
        <br>
      </td>
        <td style= "padding-bottom: 80px; width:400px"><strong>${title}</strong><br><br>
        <strong> By ${authors}</strong>
        <br>
        <strong> ${date}</strong>
        <br>
        <h4>Price : &#8377 ${price}</h4>
        <br>
